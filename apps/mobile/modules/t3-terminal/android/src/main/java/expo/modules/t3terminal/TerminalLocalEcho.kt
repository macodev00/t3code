...
internal object TerminalLocalEcho {
   ...
   * pass it, and that key is left for the remote echo.
   */
  internal const val MAX_PENDING_INPUT = 256

  /** Tail of the pty stream inspected for a secret prompt. */
  ...
  /**
   * Visible tail of a secret prompt. The keyword starts the line or follows
  ...
  fun noteLocalInput(
    ...
    if (echo == null || !canPaintAhead(state, echo, canPaint)) {
      return skipPaint(state, echo, input)
    }
  ...
  fun applyRemoteBuffer(state: TerminalEchoState, remote: String): TerminalBufferSync {
    ...
    val withOutput = state.copy(
      ...
      awaitingRemote = if (suffix.isNotEmpty()) false else state.awaitingRemote,
    )
  ...
  /**
   * True when the visible tail of pty output asks for a secret (password,
   * passphrase, passcode, or PIN). Those prompts run with echo disabled, so
   * painting the key would reveal it until the next remote frame.
   */
   ...
   * because that key can move the cursor. Already-trusted input that we
   * refused to paint (a secret, a full prediction cap, or a surface that is
   * not ready) must not block the next safe key.
   */
  private fun skipPaint(
    ...
    if (echo == null || held.echoTrusted) {
      return LocalEchoDecision(paint = null, state = held)
    }
    return LocalEchoDecision(
      paint = null,
      state = held.copy(pendingInput = rememberPending(held.pendingInput, echo)),
    )
  }
 ...
 * [awaitingRemote] is set after a control key until the pty sends new bytes,
 * so the next character is not painted at a stale cursor.
 */
