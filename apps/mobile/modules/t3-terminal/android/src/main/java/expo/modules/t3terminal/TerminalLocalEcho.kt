 ...
 * shell echoes unchanged are painted ahead. Controls, pastes, labeled
 * prompts, and any line where remote echo cannot be inferred stay on the
 * remote path. Prediction stops once the unconfirmed
...
internal object TerminalLocalEcho {
  ...
  fun looksLikePasswordPrompt(buffer: String): Boolean {
    ...
    return PASSWORD_PROMPT.containsMatchIn(visibleCurrentLine(buffer))
  }

  /**
   * True when the cursor is on a line that a cooked shell would echo.
   * Remote termios is not on this wire, so echo-off cannot be read directly.
   * Paint only at shell-style prompts (`$`, `#`, `%`, `>`). Labeled prompts
   * such as `API token: ` are left for the pty.
   */
  fun echoLikelyEnabled(buffer: String): Boolean {
    val current = visibleCurrentLine(buffer)
    val previous = visiblePreviousLine(buffer)
    val prompted = isShellPrompt(current) || lineHasShellPromptPrefix(current)
    val continuation = isShellPrompt(previous) &&
      (current.isEmpty() || !current.endsWith(':'))
    return buffer.isNotEmpty() && (prompted || continuation)
  }
   ...
   * byte is already on screen, the line looks like a shell prompt that
   * echoes, and the cursor is not sitting on a secret prompt.
   */
  private fun shouldPaint(state: TerminalEchoState): Boolean {
    ...
    return cursorReady &&
      echoLikelyEnabled(state.confirmed) &&
      !looksLikePasswordPrompt(state.confirmed)
  }
  ...
  /** Visible text after the last newline or carriage return. */
  private fun visibleCurrentLine(buffer: String): String =
    stripAnsi(buffer.substringAfterLast('\n').substringAfterLast('\r')).trimEnd()

  /** Visible text of the line before [visibleCurrentLine]. */
  private fun visiblePreviousLine(buffer: String): String {
    val lineStart = buffer.lastIndexOfAny(charArrayOf('\n', '\r'))
    if (lineStart <= 0) return ""
    val before = buffer.substring(0, lineStart).trimEnd('\n', '\r')
    return stripAnsi(before.substringAfterLast('\n').substringAfterLast('\r')).trimEnd()
  }

  /** True when [line] is a shell prompt that ends with `$`, `#`, `%`, or `>`. */
  ...
  /**
   * True when [line] has a shell prompt and then typed text (`$ ls`).
   * `$5` and `foo>bar` are not prompts.
   */
  private fun lineHasShellPromptPrefix(line: String): Boolean {
    val idx = maxOf(
      line.lastIndexOf('$'),
      line.lastIndexOf('#'),
      line.lastIndexOf('%'),
    ...
    val after = idx + 1
    return after >= line.length || line[after].isWhitespace()
  }
