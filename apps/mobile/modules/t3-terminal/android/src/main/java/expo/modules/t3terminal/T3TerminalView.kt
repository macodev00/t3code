...
class T3TerminalView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  /**
   * Sends [data] to the remote pty. Printable typing is painted only when
   * [TerminalLocalEcho.noteLocalInput] returns bytes; that requires a
   * shell-style prompt whose echo can be inferred. Echo-off prompts such as
   * `read -s -p 'API token: '` are not painted.
   */
