...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   * that query already has. A missing previous provider instance is not a live
   * session: its thread credential is revoked and cleared, so a stopped
   * process's bearer token is not restored. A probe failure revokes the
   * thread, including the token a stopped query was using. An accepted
   * replacement revokes every credential except the one it just issued.
   */
  const settleMcpCredentialAfterStart = Effect.fn("ProviderService.settleMcpCredentialAfterStart")(
    ...
    function* (input: {
      ...
      // The previous provider instance is gone, so there is no live query to
      // keep authorized. Restoring its config would leave the stopped process's
      // bearer token registered for the MCP liveness window.
