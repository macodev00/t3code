...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  // One permit per thread so a start and a recovery cannot mint or revoke
  // MCP credentials for that thread at the same time.
  const lockForThreadStart = (threadId: ThreadId) =>
    SynchronizedRef.modifyEffect(threadStartLocks, (current) => {
  ...
  // `retainExisting` leaves the thread's current bearer registered so a
  // rejected restart can keep the token the live query already baked in.
  ...
  // A failed probe is `unconfirmed`: callers must not treat it as proof the
  // session is still running.
  const probeMcpSession = (
  ...
  // The previous provider instance owns the credential when it differs from
  // the instance being started. `located` is false when that adapter is gone.
  const resolveMcpSessionOwner = (input: {
  ...
  // Keep the existing bearer while a replacement is attempted only when a
  // located owner still has the session, or the probe itself failed.
  const mcpSessionShouldRetainCredential = (input: {
  ...
  // A rejection that leaves the session up restores its credential. A missing
  // previous adapter, or a probe that cannot confirm the session, revokes the
  // thread instead of restoring a stopped process's bearer. An accepted start
  // revokes every credential except the one it just issued.
  const settleMcpCredentialAfterStart = (input: {
    readonly threadId: ThreadId;
  ...
  const recoverSessionForThread = Effect.fn("recoverSessionForThread")(function* (input: {
    ...
    // The hasSession check and any resumed start share startSession's lock.
    // A replacement can drop the adapter context before the new process is
    // registered; recovery must wait and adopt that start instead of minting
    // a second bearer.
    return yield* withThreadStartLock(
      input.binding.threadId,
      Effect.gen(function* () {
        // startSession publishes the new binding before it releases this lock.
        // A snapshot taken while waiting still names the instance that was
        // just stopped.
        const current =
          Option.getOrUndefined(yield* directory.getBinding(input.binding.threadId)) ??
          input.binding;
        const currentInstanceId = yield* requireBindingInstanceId(input.operation, current);
        const adapter = yield* registry.getByInstance(currentInstanceId);
        const hasResumeCursor = current.resumeCursor !== null && current.resumeCursor !== undefined;
        const hasActiveSession = yield* adapter.hasSession(current.threadId);
        if (hasActiveSession) {
          const activeSessions = yield* adapter.listSessions();
          const existing = activeSessions.find((session) => session.threadId === current.threadId);
          if (existing) {
            const session = { ...existing, providerInstanceId: currentInstanceId };
            yield* upsertSessionBinding(session, current.threadId);
            yield* analytics.record("provider.session.recovered", {
              provider: existing.provider,
              strategy: "adopt-existing",
              hasResumeCursor: existing.resumeCursor !== undefined,
            });
            return { adapter, session, instanceId: currentInstanceId } as const;
          }
        }
        ...
        if (!hasResumeCursor) {
          return yield* toValidationError(
            input.operation,
            `Cannot recover thread '${current.threadId}' because no provider resume state is persisted.`,
          );
        }

        const persistedCwd = readPersistedCwd(current.runtimePayload);
        const persistedModelSelection = readPersistedModelSelection(current.runtimePayload);

        yield* prepareMcpSession(current.threadId, currentInstanceId);
        const resumed = yield* adapter
          .startSession({
            ...
            ...(hasResumeCursor ? { resumeCursor: current.resumeCursor } : {}),
            runtimeMode: current.runtimeMode ?? "full-access",
          })
          .pipe(Effect.onError(() => clearMcpSession(current.threadId)));
        if (resumed.provider !== adapter.provider) {
          yield* clearMcpSession(current.threadId);
          return yield* toValidationError(
            input.operation,
            `Adapter/provider mismatch while recovering thread '${current.threadId}'. Expected '${adapter.provider}', received '${resumed.provider}'.`,
          );
        }

        const session = { ...resumed, providerInstanceId: currentInstanceId };
        yield* upsertSessionBinding(session, current.threadId);
        yield* analytics.record("provider.session.recovered", {
      ...
      return yield* Effect.gen(function* () {
        ...
        const sessionWithInstance = yield* withThreadStartLock(
          ...
          Effect.gen(function* () {
            ...
            // Publish the new instance before releasing the lock. Recovery
            // re-reads the binding once it acquires the same permit.
