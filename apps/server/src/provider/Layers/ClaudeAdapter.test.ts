...
function makeQueryRecordingClaudeLayer() {
...
describe("ClaudeAdapterLive", () => {
  it.effect("does not replace a session while live tasks are still running", () => {
    const harness = makeQueryRecordingClaudeLayer();
    return Effect.gen(function* () {
      ...
      assert.equal(replaceError._tag, "ProviderAdapterRequestError");
      if (!(replaceError instanceof ProviderAdapterRequestError)) {
        return;
      }
      ...
      if (warnings[0]?.type === "runtime.warning") {
        assert.match(warnings[0].payload.message, /live task/);
      }
    }).pipe(
  ...
  it.effect("does not roll a session back while live tasks are still running", () => {
    const harness = makeQueryRecordingClaudeLayer();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const taskStartedFiber = yield* adapter.streamEvents.pipe(
      ...
      harness.queries[0]?.emit({
        ...
        description: "Agent A",
        task_type: "local_agent",
        uuid: "task-rollback-uuid",
        session_id: "sdk-session",
      } as unknown as SDKMessage);
      yield* Fiber.join(taskStartedFiber);

      const rollbackError = yield* adapter.rollbackThread(THREAD_ID, 1).pipe(Effect.flip);

      assert.equal(rollbackError._tag, "ProviderAdapterRequestError");
      if (!(rollbackError instanceof ProviderAdapterRequestError)) {
        return;
      }
      assert.equal(rollbackError.method, "thread/rollback");
      assert.match(rollbackError.detail, /live task/);
      assert.equal(harness.queries.length, 1);
      assert.equal(harness.queries[0]?.closeCalls, 0);
      assert.equal(yield* adapter.hasSession(THREAD_ID), true);
      assert.equal((yield* adapter.listSessions())[0]?.runtimeMode, "full-access");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("replaces a session after its live tasks have finished", () => {
    const harness = makeQueryRecordingClaudeLayer();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const taskStartedFiber = yield* adapter.streamEvents.pipe(
      ...
      assert.equal(yield* adapter.hasSession(THREAD_ID), true);
      assert.equal(activeSessions.length, 1);
      assert.equal(activeSessions[0]?.runtimeMode, "approval-required");
