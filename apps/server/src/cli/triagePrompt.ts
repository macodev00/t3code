...
\`GET <origin>/.well-known/t3/environment\` (unauthenticated). The context file's
...
export interface TriageContextInput {
  ...
  /** Version of the `t3` process that wrote this file. Not the product the bug happened on. */
  readonly cliVersion: string;
  readonly cliReleaseTag: string;
  /**
   * `serverVersion` from the live local server, or a short reason none was read.
   * Desktop and service installs publish this on `/.well-known/t3/environment`.
   */
  readonly localServerVersion: string;
  readonly localServerReleaseTag?: string;
  readonly os: string;
...
- Triage CLI version: ${input.cliVersion}
- Release tag for the triage CLI: ${input.cliReleaseTag}
- Local server version: ${input.localServerVersion}${
  input.localServerReleaseTag === undefined
    ? ""
    : `\n- Release tag for the local server: ${input.localServerReleaseTag}`
}
- OS: ${input.os}
