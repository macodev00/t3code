...
it.effect("revokes every superseded thread credential except the accepted provider session", () => {
  ...
  return Effect.gen(function* () {
    ...
    const issue = (id: ThreadId, retainExisting: boolean) =>
    ...
    const bearer = (header: string | undefined) => header?.replace(/^Bearer\s+/, "") ?? "";

    const first = yield* issue(threadId, false);
    const replaced = yield* issue(threadId, false);
    const kept = yield* issue(otherThreadId, false);
    if (first === undefined || replaced === undefined || kept === undefined) {
      return yield* Effect.die("expected MCP credentials");
    }
    const firstToken = bearer(first.config.authorizationHeader);
    const replacedToken = bearer(replaced.config.authorizationHeader);
    expect(yield* registry.resolve(firstToken)).toBeUndefined();
    expect((yield* registry.resolve(replacedToken))?.threadId).toBe(threadId);

    const retained = yield* issue(threadId, true);
    const superseded = yield* issue(threadId, true);
    if (retained === undefined || superseded === undefined) {
      return yield* Effect.die("expected retained MCP credentials");
    }
    const retainedToken = bearer(retained.config.authorizationHeader);
    const supersededToken = bearer(superseded.config.authorizationHeader);
    const keptToken = bearer(kept.config.authorizationHeader);
