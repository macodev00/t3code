...
projectionSnapshotLayer("ProjectionSnapshotQuery windowed thread detail", (it) => {
  ...
  it.effect("preserves agent lifecycle outside the turn window and activity cap", () =>
    Effect.gen(function* () {
      ...
      const assertAgents = (activities: ReadonlyArray<{ id: string; payload?: unknown }>) => {
        ...
        for (const activity of activities) {
          ...
          if (payload.agentKind === "agent" && typeof payload.taskId === "string") {
            agentIds.add(payload.taskId);
          }
          const usage = payload.typedUsage;
        ...
        assert.equal(totalTokens, 17_000);
      };
